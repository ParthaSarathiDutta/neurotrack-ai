  ...
  for (const label of ['test50', 'test51', 'test53']) {
    ...
    await page.waitForFunction(
      () => {
        ...
        return msg.includes('Detected') || msg.includes('failed') || msg.includes('error');
      },
      { timeout: 180_000 },
    );
    ...
    await page.waitForFunction(
      () => document.querySelector('[data-testid="proposed-start"]')?.textContent?.includes('Proposed'),
      { timeout: 120_000 },
    );
    const proposed = await page.locator('[data-testid="proposed-start"]').textContent();
